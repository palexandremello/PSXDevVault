...
void delay(int iterations) {
  for (int i = 0; i < iterations; i++) {
    // Loop vazio para adicionar atraso
  }
}
...
void draw_text(RenderContext *ctx, int x, int y, int z, const char *text) {
  ...
  ctx->next_packet =
      (uint8_t *)FntSort(&(buffer->ot[z]), ctx->next_packet, x, y, text);
...
int main(int argc, const char **argv) {
  ...
  for (;;) {
    ...
    TILE *tile2 = (TILE *)new_primitive(&ctx, 0, sizeof(TILE));
    ...
    setTile(tile2);
    setXY0(tile2, SCREEN_XRES / 2, SCREEN_YRES / 2);
    setWH(tile2, 64, 64);
    setRGB0(tile2, 0, 255, 0);
